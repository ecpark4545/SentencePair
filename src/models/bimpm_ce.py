...
class BIMPMCrossEntropy(nn.Module):
	def __init__(self, hparams):
		...
		# Matching layer
		for i in range(1, 9):
			setattr(self, f'mp_w{i}', nn.Parameter(torch.rand(self.num_layer, self.hidden_size)))
			
		
		
		# Prediction layer
		self.classification = nn.Sequential(self.dropout, nn.Linear(4 * self.hidden_size, self.hidden_size * 2),
		                                    nn.Tanh(), self.dropout, nn.Linear(self.hidden_size * 2, 2))
	...
	def forward(self, pre: torch.Tensor, hypo: torch.Tensor) -> torch.Tensor:
		...
		att_mean_pre_fw = self.div_with_small_value(att_pre_fw.sum(dim=1),att_fw.sum(dim=1, keepdim=True).permute(0, 2, 1).contiguous())
		att_mean_pre_bw = self.div_with_small_value(att_pre_bw.sum(dim=1),att_bw.sum(dim=1, keepdim=True).permute(0, 2, 1).contiguous())
