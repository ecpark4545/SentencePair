...
class CheckpointManager(object):
  ...
  def __init__(self,model,optimizer,checkpoint_dirpath,step_size=1,last_epoch=-1,**kwargs):
  ...
  def init_directory(self, config):
    ...
    # with open(str(self.ckpt_dirpath / "config.json"), 'w') as config_handle:
  ...
  def step(self, epoch=None):
    ...
    if not self.last_epoch % self.step_size:
      torch.save(
        ...
        # self.ckpt_dirpath / f"checkpoint.pth",
      )
